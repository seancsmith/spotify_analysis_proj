...
sample_index <- sample(1:n_data, size = n_data*0.5)
...
```{r}
model_genres <- lm(popularity ~ year + danceability + loudness + liveness + explicit, 
...
```{r}
spotify_genres_modelling %>% 
  ggplot() +
  aes(x = top_genres, y = popularity) +
  geom_boxplot()
```
